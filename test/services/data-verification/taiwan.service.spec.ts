import { TaiwanDataVerificationService } from '../../../src/services/data-verification/taiwan.service';

describe('TaiwanDataVerificationService', () => {
  let service: TaiwanDataVerificationService;

  beforeEach(() => {
    service = new TaiwanDataVerificationService({
      apiKey: 'test-api-key',
      appId: 'test-app-id',
  ...
  describe('getCompanyNumberTaiwan', () => {
    it('should call post with the mapped endpoint', async () => {
      const mockResponse = { status: true, data: {} };
      jest.spyOn(service as any, 'post').mockResolvedValue(mockResponse);

      const result = await service.getCompanyNumberTaiwan({ sample: true });
